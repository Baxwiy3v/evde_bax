    ...
    public class DetailController : Controller
    {
		...
		public async Task<IActionResult> Detail(int id)
		{
            ...
            Product product = await _context.Products.Include(p => p.Category)
                .Include(p => p.ProductImages).Include(p => p.ProductTags).ThenInclude(p => p.Tag)
            ...
            List<Product> RelatedProducts = await _context.Products.Include(pi => pi.ProductImages.Where(pi => pi.IsPrimary != null))
                .Where(p => p.CategoryId == product.CategoryId && p.Id != product.Id).ToListAsync();
